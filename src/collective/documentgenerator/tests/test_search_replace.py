...
class TestSearchReplaceTemplate(PODTemplateIntegrationTest):
    ...
    def test_can_search_in_headers_and_footers(self):
        ...
        self.assertIn(
            "context.title.upper()", header_results[self.template2.UID()][0].content
        )

        self.assertEqual(len(footer_results[self.template2.UID()]), 1)
        self.assertIn(
            "view.display_date", footer_results[self.template2.UID()][0].content
        )
    ...
    def test_can_search_multiple_times(self):
        ...
        self.assertIn(
            "view.get_localized_field_name", regex_results[template1_uid][0].content
        )
    ...
    def test_replace_dont_change_podtemplate_file_size_too_much(self):
        ...
        self.assertAlmostEqual(
            before_template1_filesize, self.template1.odt_file.getSize(), delta=1000
        )
        self.assertAlmostEqual(
            before_template2_filesize, self.template2.odt_file.getSize(), delta=1000
        )
    ...
    def test_can_replace_in_ods(self):
        with SearchAndReplacePODTemplates((self.ods_template,)) as search_replace:
            ...
            # At the regex start, '^' was failing before
            regex_start_results = search_replace.replace(
                "^do row", "do this", is_regex=True
            )
        ...
        self.assertIn(
            "regex are evil", regex_search[self.ods_template.UID()][0].content
        )
    ...
    def test_replace_can_replace_in_headers_and_footers(self):
        ...
        self.assertIn(
            "context.title.lower()", header_results[self.template2.UID()][0].patched
        )
        self.assertIn(
            "view.new_method(", footer_results[self.template2.UID()][0].patched
        )
        ...
        self.assertEqual(
            "context.title.lower()", header_results[self.template2.UID()][0].keyword
        )

        self.assertGreater(len(footer_results), 0)
        self.assertEqual(
            "view.new_method(", footer_results[self.template2.UID()][0].keyword
        )
    ...
    def test_search_replace_control_panel_can_preview(self):
        view = self.portal.restrictedTraverse(
            "@@collective.documentgenerator-searchreplacepanel"
        )
    ...
    def test_search_replace_control_panel_regex_validator(self):
        ...
        self.assertEqual(
            errors[0].args[0], u'Incorrect regex at row #2 : "get_elements("'
        )
        replacements = [
            {"search_expr": "get_elements(", "replace_expr": "", "is_regex": False},
