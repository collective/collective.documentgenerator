...
class TestConfigurablePODTemplateIntegration(ConfigurablePODTemplateIntegrationTest):
    ...
    def test_validate_pod_generation_context_variables(self):
        class Dummy(object):
        ...
        # check forbidden name
        data = Dummy(merge_templates=[{'name': u'uids', 'value': u'1'}])
        self.assertRaises(Invalid, fct, data)
        # check duplicated name
        data = Dummy(merge_templates=[{'name': u'det', 'value': u'1'}, {'name': u'det', 'value': u'1'}])
        self.assertRaises(Invalid, fct, data)
        # no exception
        data = Dummy(merge_templates=[{'name': u'det', 'value': u'1'}])
        self.assertIsNone(fct(data))

        # check both variables but sub template present
        # check forbidden name
        data = Dummy(context_variables=[{'name': u'det', 'value': u'1'}],
                     merge_templates=[{'name': u'uids', 'value': u'1'}])
        self.assertRaises(Invalid, fct, data)
        # revert
        data = Dummy(merge_templates=[{'name': u'det', 'value': u'1'}],
        ...
        self.assertRaises(Invalid, fct, data)
        # check duplicated name
        data = Dummy(merge_templates=[{'name': u'same', 'value': u'1'}, {'name': u'same', 'value': u'1'}],
        ...
        data = Dummy(context_variables=[{'name': u'same', 'value': u'1'}, {'name': u'same', 'value': u'1'}],
                     merge_templates=[{'name': u'random1', 'value': u'1'}, {'name': u'random2', 'value': u'1'}])
        self.assertRaises(Invalid, fct, data)
        ...
        data = Dummy(merge_templates=[{'name': u'same', 'value': u'1'}, {'name': u'random1', 'value': u'1'}],
